...
unsigned char psx_parallel_out_0 = 0xff;
unsigned char psx_parallel_out_2 = 0x00;
...
void psx_att(int base, int conport, int on)
{
   ...
   unsigned char att_array[] = { LPT_D1, LPT_D1, LPT_D3, LPT_D4, LPT_D5, LPT_D6, LPT_D7 };
   unsigned char att;
 ...
 *
 */
unsigned char psx_sendbyte(int base, int conport, int delay, unsigned char byte, int wait)
{
   int i;
   unsigned char data;
...
int psx_obtain_io_permission(int base)
{
	...
	if (already==1) { return 1; }
    // return 1 if successful, otherwise, 0
	#ifdef _PSX_LINUX_
	if (ioperm(base,3,1)) { return 0; }
	#endif
	already=1;
	return 1;
}
...
PSX_CON_BUF* psx_controller_read(int base, int conport, int tap, int delay)
{
   ...
   /* check the controller has a legal id */
   if (!((psx_con_buf.type > 0) && (psx_con_buf.type < 0x0f) && (psx_con_buf.type != 0x08)))
   {
      psx_sendclose(base, conport, delay);
      return NULL;
   }
 ...
 *
 */
void psx_controller_vshock(int base, int conport, int tap, int delay, int shock, int rumble)
{
 ...
 *
 */
char* psx_memcard_read_frame(int base, int conport, int tap, int delay, int frame)
{
   int i, xor;
 ...
 *
 */
int psx_memcard_write_frame(int base, int conport, int tap, int delay, int frame, char* data_f)
{
   int i, xor;
   unsigned char c_data;
   
   unsigned char cmd_wstring_hdr[4] = {(0x80 + (unsigned char)tap), 0x57, 0x00, 0x00};
 ...
 *
 */
char* psx_memcard_read_block(int base, int conport, int tap, int delay, int block)
{
   int i, j;
 ...
 *
 */
int psx_memcard_write_block(int base, int conport, int tap, int delay, int block, char* data_b)
{
   int i, xor;
   
   for (i = 0; i < 64; i++)
   {
      xor = psx_memcard_write_frame(base, conport, tap, delay, (block*64)+i, &(data_b[128*i]));
 ...
 *
 */
PSX_MCB_INFO_DIR* psx_mcb_read_dir(int base, int conport, int tap, int delay, int block)
{
   int i, xor;
 ...
 *
 */
PSX_MCB_INFO_DAT* psx_mcb_read_dat(int base, int conport, int tap, int delay, int block)
{
   int i, j;
   unsigned char *data_f;
 ...
 *
 */
PSX_MCB_INFO* psx_mcb_info_merge(PSX_MCB_INFO_DIR mcb_info_dir, PSX_MCB_INFO_DAT mcb_info_dat, PSX_MCB_INFO *mcb_info)
{
   mcb_info->read = 1;
 ...
 *
 */
PSX_MCB_INFO* psx_mcb_read_info(int base, int conport, int tap, int delay, int block)
{
